...
Пример создания записи в справочнике Упаковка:
- METHOD: POST
...
### Расходная накладная (валовая відборка та завантаження)
...
```json
{
"document_id": "e4e03f2b0505692cdbe",
"document_document_type_id": "LEAVE_DELIVERY",
"document_warehouse_id": "dd7e8bdd8d385b75188",
"document_vehicle_number": "AА 2967 ІК",
"document_document_priority_id": "0",
"document_comment": "РЦ28",
"document_title": "Маршрутний лист BKV00005448 от 05.07.2024 0:00:00",
"orders": [
{
"document_order_id": "8bed2a4100505692cdbe",
"document_order_title": "Заказ покупателя 141074 от 05.07.2024 0:04:01",
"document_order_warehouse_id": "2c220ac0d8d385b75188",
"document_order_ordinal_number": 1,
"items": [
{
"document_det_product_id": "0db24080005056b68d7c",
"document_det_amount": 3560
},
{
"document_det_product_id": "26d2834a00505692b746",
"document_det_amount": 36
}
]
},
{
"document_order_id": "fd57b5a000505692cdbe",
"document_order_title": "Заказ покупателя 141163 от 05.07.2024 0:04:14",
"document_order_warehouse_id": "2c220ac0d8d385b75188",
"document_order_ordinal_number": 1,
"items": [
{
"document_det_product_id": "c30da5cd00505692d69f",
"document_det_amount": 720
}
]
}
]
...
### Расходная накладная (по точкам відборка та завантаження)
> document_document_type_id = LEAVE_BY_POINTS
...
```json
{
    "document_id": "8542c17400505692cdbe",
    "document_document_type_id": "LEAVE_BY_POINTS",
    "document_warehouse_id": "dd7e8bddd8d385b75188",
    "document_vehicle_number": "АІ 6753 НХ",
    "document_document_priority_id": "19",
    "document_comment": "ПОТОЧКА",
    "document_title": "Маршрутний лист BKV00005409 от 04.07.2024 23:59:59",
    "orders": [
        {
            "document_order_id": "0269f52600505692cdbe",
            "document_order_title": "Заказ покупателя 138761 от 04.07.2024 0:36:45",
            "document_order_warehouse_id": "dd7e8bdd8d385b75188",
            "document_order_ordinal_number": 1,
            "items": [
                {
                    "document_det_product_id": "18050d9700505692d69f",
                    "document_det_amount": 30
                },
                {
                    "document_det_product_id": "5a384074005056b697c1",
                    "document_det_amount": 8
                }
            ]
        },
        {
            "document_order_id": "6beb9bb800505692cdbe",
            "document_order_title": "Заказ покупателя 140243 от 04.07.2024 0:39:46",
            "document_order_warehouse_id": "dd7e8bddd8d385b75188",
            "document_order_ordinal_number": 2,
            "items": [
                {
                    "document_det_product_id": "18050d9700505692d69f",
                    "document_det_amount": 45
                },
                {
                    "document_det_product_id": "5a384074005056b697c1",
                    "document_det_amount": 12
                }
            ]
        }
    ]
...
>
> В данный момент доступны только прописанные тестовые склады
...
### Контрагенты Новая Почта (Получатели)
...
### Список доступных складов Новой Почты
Список доступных складов новой почты являются частью документооборота, 
но работают по принципу обычных справочников `Справочники v2 (CRUD/PSC)`
...
### Статусы почтовых операторов. Экспорт
...
Наименование|Тип|R|Зависимость|Описание
---|---|---|---|---
document_np_id|varchar(255)|-|document|Идентификатор документа привязан к документу из таблицы document
document_np_service_type|varchar(50)|-||Тип доставки новой почты
document_np_payment_method|varchar(50)|-||Тип оплаты новой почты
document_np_payer_type|varchar(50)|-||Тип плательщика новой почты
document_np_cost|varchar(50)|-||Объявленная стоимость
document_np_seats_amount|varchar(10)|-||Кол-во грузомест
document_np_desc|varchar(255)|-||Дополнительное описание
document_np_cargo_type|varchar(50)|-||Тип груза новой почты
document_np_sender_f|varchar(50)|-||Фамилия отправителя
document_np_sender_i|varchar(50)|-||Имя отправителя
document_np_sender_o|varchar(50)|-||Отчество отправителя
document_np_sender_city|varchar(50)|-||Город отправителя
document_np_sender_region|varchar(50)|-||Регион отправителя
document_np_sender_phone|varchar(50)|-||Номер отправителя
document_np_sender_address|varchar(100)|-||Адрес отправителя
document_np_sender_warehouse|varchar(100)|-||Склад отправителя
document_np_receiver_f|varchar(50)|-||Фамилия получателя
document_np_receiver_i|varchar(50)|-||Имя получателя
document_np_receiver_o|varchar(50)|-||Отчество получателя
document_np_receiver_phone|varchar(50)|-||Номер получателя
document_np_receiver_city|varchar(50)|-||Город получателя
document_np_receiver_region|varchar(50)|-||Регион получателя
document_np_receiver_address|varchar(100)|-||Адрес получателя
document_np_receiver_warehouse|varchar(100)|-||Склад получателя

## Содержание справочников (ENUM)
### counterparty_type
Значение|Описание
---|---
...
Значение|Описание
---|---
DoorsDoors|Адрес > Адрес
DoorsWarehouse|Адрес > Склад
WarehouseDoors|Склад > Адрес
